...
import 'package:dairy_newdeskapp/model/StockFilterGetResponse.dart';
...
class StockUpdateState extends State<StockUpdate> {
  ...
  void getMilkCollections(int user_id,int page) async {
    ...
    http.Response response = await http.post(ApiBaseUrl.base_url +"api/stockupdates_list", body: data);
    if (response.statusCode == 200)
    {
      ...
      if (jsonResponse["status"] == 200) {
        ...
        user1 = new StockListResponse.fromJson(jsonResponse);
  ...
  void nextscreen(int id) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => EditStockSale(id)),
    );
  }
  ...
  Future<void> getFilterList(GlobalKey<State<StatefulWidget>> keyLoader,
    ...
    try {
      ...
      final responseData = await _dio.post<Map<String, dynamic>>(
          '/api/stockupdatefilter',
          options: RequestOptions(
              method: 'POST',
              headers: <String, dynamic>{},
              baseUrl: ApiBaseUrl.base_url),
          data: formData);
      user2 = StockFilterGetResponse.fromJson(responseData.data);
      setState(() {
        type = 'filter';
        resultvalue2 = true;
  ...
  _launchURL() async {
    String url =
        'http://shankraresearch.com/stock_update'+_userId.toString();
