import 'dart:async';
import 'dart:convert';
import 'package:dairy_newdeskapp/model/AddDeductionResponse.dart';
import 'package:dairy_newdeskapp/model/DeductListResponse.dart';
...
import 'EditDeduct.dart';

class Deduct extends StatefulWidget {
  @override
  DeductState createState() => DeductState();
}

class DeductState extends State<Deduct> {
  ...
  Future<void> addMilkCollection(
    ...
    try {
      ...
      AddDeductionResponse user =
          AddDeductionResponse.fromJson(responseData.data);
      if (user.status == 200) {
        ...
        Controllers.cow_min_fat.text = '';
        Controllers.cow_per_unit.text = '';
        Controllers.cow_cost.text = '';
        Controllers.cow1_min_snf.text = '';
        Controllers.cow1_per_unit.text = '';
        Controllers.cow1_cost.text = '';
        Controllers.buf_min_fat.text = '';
        Controllers.buf_per_unit.text = '';
        Controllers.buf_cost.text = '';
  ...
  void getMilkCollections(int user_id,int page) async {
    ...
    http.Response response = await http.post(ApiBaseUrl.base_url +"api/deducations_list", body: data);
    if (response.statusCode == 200)
    {
      ...
      if (jsonResponse["status"] == 200) {
        user1 = new DeductListResponse.fromJson(jsonResponse);
  ...
  void nextscreen(int id) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => EditDeduct(id)),
    );
  }
  ...
  _launchPdfURL() async {
    String url =
        'https://shankraresearch.com/deduct/' + _userId.toString();
  ...
  _launchURL() async {
    String url =
        'http://shankraresearch.com/deductExcel'+_userId.toString();
    if (await canLaunch(url)) {
      await launch(url);
    } else {
      throw 'Could not launch $url';
