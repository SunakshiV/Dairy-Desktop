...
import 'EditStockResponse.dart';
import 'GetStockResponse.dart';
import 'package:get/get.dart';

// ignore: must_be_immutable
class EditStockSale extends StatefulWidget {
  int _id;
  EditStockSale(int id) {
    this._id = id;
  }
  @override
  EditStockSaleState createState() => EditStockSaleState();
}

class EditStockSaleState extends State<EditStockSale> {
  int select, valueRadio;
  TimeOfDay selectedTime = TimeOfDay.now();
  var value = false;
  GetStockResponse user1;
  ...
  Future<void> addMilkCollection(
    ...
    final formData = {
      ...
      'type': Controllers.edtmilk_weight.text,
      'vendor_number': Controllers.edtmilk_vendor_Code.text,
      'vendor_name': Controllers.edtmilk_vendor_name.text,
      'amount': Controllers.edtmilk_amount.text,
      'quantity': Controllers.edtmilk_rate.text,
      'id': widget._id
    };
    print("date" + dateController2.text);
    print("type" + Controllers.edtmilk_weight.text);

    print("vendorname" + Controllers.edtmilk_vendor_name.text);
    print("amount" + Controllers.edtmilk_amount.text);
    print("quantity" + Controllers.edtmilk_rate.text);
    print("idsssssssssssssssss" + widget._id.toString());
    ...
    try {
      ...
      EditStockResponse user = EditStockResponse.fromJson(responseData.data);
      if (user.status == 200) {
        getAccounts(_keyLoader, widget._id);
  ...
  Future<void> getAccounts(
    ...
    try {
      ...
      final responseData = await _dio.post<Map<String, dynamic>>(
          '/api/stockupdatesingle',
          options: RequestOptions(
              method: 'POST',
              headers: <String, dynamic>{},
              baseUrl: ApiBaseUrl.base_url),
          data: formData);
      user1 = GetStockResponse.fromJson(responseData.data);
      ...
      Controllers.edtmilk_weight.text = user1.data.elementAt(0).type;
      Controllers.edtmilk_rate.text = user1.data.elementAt(0).quantity;
